...
def computeJK(C,I,b_mnl,p):
    ...
    if(not p.RI):
        if(not p.gpu):
            ...
            #hstarj
            #J = cp.einsum('isl,mnsl->imn', D, I, optimize=True)
            J = cp.tensordot(D, I, axes=([1,2],[2,3]))

            #hstark
            #K = cp.einsum('inl,mnsl->ims', D, I, optimize=True)
...
def computeJKH_core_MO(C,H,I,b_mnl,p):
    ...
    if (not p.RI):
        if (not p.gpu):
            ...
            #J = cp.einsum('isl,mnsl->imn', D, I, optimize=True)
            J = cp.tensordot(D, I, axes=([1,2],[2,3]))
            J_MO = cp.einsum('jmn,imn->ij', D, J, optimize=True)
            #J_MO = cp.tensordot(J, D,axes=((1,2),(1,2)))
    
            #QKMATm
            #K = cp.einsum('inl,mnsl->ims', D, I, optimize=True)
            K = cp.tensordot(D, I, axes=([1,2],[1,3]))
            K_MO = cp.einsum('jms,ims->ij', D, K, optimize=True)
